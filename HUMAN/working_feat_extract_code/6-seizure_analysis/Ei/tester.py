...
target = data_filt[:, target_range[0]:target_range[1]]
base = data_filt[:, bl_range[0]:bl_range[1]]
...
print(onset_location)
plt.plot(un - np.minimum.accumulate(un))
# %%
#testing out the scipy package
...
for i in range(len(ER)):
    ...
    if ph.detected_change():
        print('Change has been detected in data: ' + str(ER[i]) + ' - of index: ' + str(i))
        print('U_N - u_n:', (un - np.minimum.accumulate(un))[i])
# %%
