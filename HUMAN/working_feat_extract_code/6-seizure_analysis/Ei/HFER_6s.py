...
def compute_ei_index(target, base, fs):
    ...
    hfer = np.sum(target[:, (int(39)*fs):(int(60-15)*fs)], axis=1) / (fs * int(21-15))
...
results_df.to_csv('../data/self_run/hfer_HUP_6s.csv', index=False)
...
results_df.to_csv('../data/self_run/hfer_MUSC_6s.csv', index=False)
print("All results saved.")
