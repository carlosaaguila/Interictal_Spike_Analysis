...
def process_single_patient(row, session, pw_path, ieeg_filename_df):
    ...
    bl_start = (start_time - 180)
    bl_end = (start_time - 120)
    target_end = (start_time + 0)
...
results_df.to_csv('../data/self_run/EI_new_onset_detector/EI_HUP_120_120s.csv', index=False)
...
def process_single_patient(row, session, pw_path, ieeg_filename_df):
    ...
    bl_start = (start_time - 120)
    bl_end = (start_time - 80)
...
results_df = pd.DataFrame([result for result in all_results if result is not None])
results_df.to_csv('../data/self_run/EI_new_onset_detector/EI_MUSC_120_120s.csv', index=False)
print("All results saved.")