...
def determine_threshold_onset(target, base):
    ...
    for channel_idx in range(target_data.shape[0]):
        ...
        MA = list(map(lambda x: cum_sum_avg(ER,x),np.arange(len(ER))))
        un = np.cumsum(ER[1:]-MA[1:]-np.repeat(0.5, np.shape(ER[1:])))
        thresh = 1000
...
results_df.to_csv('../data/self_run/EI_new_onset_detector/EI_HUP_v0-5.csv', index=False)
...
results_df = pd.DataFrame([result for result in all_results if result is not None])
results_df.to_csv('../data/self_run/EI_new_onset_detector/EI_MUSC_v0-5.csv', index=False)
