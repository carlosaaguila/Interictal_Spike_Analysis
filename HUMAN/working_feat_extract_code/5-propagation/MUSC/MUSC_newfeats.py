...
# interval_count = all_spikes.groupby(['pt_id'])['interval number'].n_unique()
interval_count = all_spikes.groupby(['pt_id'])['interval number'].unique()
interval_count = interval_count.reset_index()
interval_count['length'] = interval_count['interval number'].apply(lambda x: len(x))
interval_count['max'] = interval_count['interval number'].apply(lambda x: max(x))
...
spike_count['spike_rate'] = spike_count['spike_count']/spike_count['max']
...
all_spikes['seq_spike_time_diff_full'] = all_spikes.groupby(['new_spike_seq','pt_id'])['peak_index_samples'].transform(lambda x: x.sort_values().diff())
...
all_spikes['recruitment_latency_full'] = all_spikes.groupby(['new_spike_seq','pt_id'])['peak_index_samples'].transform(lambda x: x.sort_values() - x.sort_values()[x.sort_values().index[0]])
...
all_spikes.to_csv('../dataset/complete_dfs/MUSC_full.csv')
...
#calculate retention_latency
#for each new_spike_seq, order them by peak_index in ascending order, and then calculate the difference between each peak_index
all_spikes['seq_spike_time_diff_thresh'] = all_spikes.groupby(['new_spike_seq','pt_id'])['peak_index_samples'].transform(lambda x: x.sort_values().diff())

#for each new_spike_seq, order them by peak_index in ascending order, and calculate the difference between each peak_index and the first peak_index denoted by is_spike_leader
all_spikes['recruitment_latency_thresh'] = all_spikes.groupby(['new_spike_seq','pt_id'])['peak_index_samples'].transform(lambda x: x.sort_values() - x.sort_values()[x.sort_values().index[0]])
...
all_spikes.to_csv('../dataset/complete_dfs/MUSC_thresholded.csv')