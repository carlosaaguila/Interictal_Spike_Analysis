...
default_output_dir = os.path.join(os.getcwd(), "ML_results", "6_sets")
...
def load_data():
    ...
    spike_rate_df = pd.read_csv("/users/aguilac/Interictal_Spike_Analysis/HUMAN/working_feat_extract_code/5-propagation/Revisions/data/mtl_spikerates.csv")
    spike_rate_df['pt_id'] = spike_rate_df['pt_id'].str.replace('3T_MP0', '').str.replace('HUP', '').astype(int)  # Ensure consistent typing
    ...
    MTL_interictal_df = pd.read_csv('/users/aguilac/Interictal_Spike_Analysis/HUMAN/working_feat_extract_code/5-propagation/Revisions/data/mtl_all_feats.csv', index_col=0)
    MTL_interictal_df['pt_id'] = MTL_interictal_df['pt_id'].str.replace('3T_MP0', '').str.replace('HUP', '').astype(int)  # Ensure consistent typing

    return combined_df, EI_df, pearson_df, spike_rate_df, spike_corr_df, MTL_interictal_df
...
def main():
    ...
    combined_data, ictal_data, interictal_data, spike_rate_data, spike_corr_data, MTL_interictal_data = load_data()  # Unpack new dataset
    ...
    datasets = [
        ...
        ("MTL Spike Rate", spike_rate_data[['spike_rate_two_mean', 'SOZ', 'pt_id']].drop(columns=['SOZ', 'pt_id']), '#4DBBD5FF'),
        ("MTL Interictal", MTL_interictal_data.drop(columns=['SOZ', 'pt_id']), '#FF7F00FF'),
        ("Pearson - Spike Rate", spike_corr_data[['spike_rate_corr', 'SOZ', 'pt_id']].drop(columns=['SOZ', 'pt_id']), '#DB3A34FF')
    ]
