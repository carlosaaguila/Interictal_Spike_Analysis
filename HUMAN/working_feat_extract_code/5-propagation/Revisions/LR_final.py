...
default_output_dir = os.path.join(os.getcwd(), "ML_results", "final")
...
def load_data():
    ...
    MTL_spike_rate_df = pd.read_csv('/users/aguilac/Interictal_Spike_Analysis/HUMAN/working_feat_extract_code/5-propagation/Revisions/data/MTL-localized-spikerate.csv', index_col=0)
    ...
    MTL_spike_rate_df['SOZ'] = MTL_spike_rate_df['SOZ'].replace({2: 0, 3: 0})

    # MTL_spike_rate_df = MTL_interictal_df[['spike_rate_two_mean','pt_id','SOZ']]
    ...
    return combined_df, EI_df, pearson_df, MTL_spike_rate_df, gamma_inter_df
...
def main():
    ...
    combined_data, ictal_data, interictal_data, MTL_spike_rate_df, gamma_inter_df = load_data()
    # Prepare datasets
    combined_features = combined_data
    interictal_features = interictal_data
    ictal_features = combined_data[['correlation', 'SOZ', 'pt_id']]

    #add this to see what's up with the combination???
    gamma_combo = gamma_inter_df.merge(combined_data[['pt_id','correlation']], on='pt_id', how='right')

    #pt's we want
    pts = combined_features.pt_id.unique()

    # datasets = [
    #     ("Combined", combined_features, '#E64B35FF'),
    #     ("Interictal", interictal_features, '#7E6148FF'),
    #     ("Ictal", ictal_features, '#00A087FF'),
    #     ("MTL Spike Rate", MTL_spike_rate_df, '#4DBBD5FF'),
    #     # ("MTL Interictal", MTL_spike_rate_df, '#FF7F00FF'),
    ...
    pr_ax.legend(loc="lower right", prop={'size': 16, 'weight': 'bold'})
