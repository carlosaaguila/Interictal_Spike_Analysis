...
def relabel(group):
    # Calculate the starting number of the next group
    start_num = (group['num'].max() // 3 + 1) * 3
    # Generate the new labels
    new_labels = group['char'] + start_num.astype(str)
    return new_labels

# Apply the function to each group
all_spikes_avg['new_label'] = all_spikes_avg.groupby('char').apply(relabel).reset_index(drop=True)

#%%
all_spikes_avg_test = all_spikes_avg.pivot_table(index=['pt_id','SOZ'], columns='char', values=Feat_of_interest)
