...
def plot_train(train, eeg_data):
    ...
    for i, SS in train.iterrows():
        ...
        if ch_to_plot in eeg_data.columns.to_list():
            ...
            axs[i].plot(range(lower_bound_idx, upper_bound_idx), eeg_data[ch_to_plot].iloc[lower_bound_idx:upper_bound_idx], color = 'k', lw = 3)
            axs[i].get_xaxis().set_visible(False)
            axs[i].plot(PEAK, eeg_data[ch_to_plot].iloc[PEAK], 'o', color = '#00a087', markersize = 7 )
        ...
        sns.despine()
        plt.tight_layout()
        plt.savefig('figures/MUSC+HUP/official/example_train.pdf')
#%%
#get a patient row.
row = pt_ids.iloc[2]
pt_id = row['hup_id']
